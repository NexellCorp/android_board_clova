...
DTB_IMG=${KERNEL_DIR}/arch/arm/boot/dts/s5p4418-clova-rev00.dtb
...
if [ "${BUILD_ALL}" == "true" ] || [ "${BUILD_BL1}" == "true" ]; then
	build_bl1_s5p4418 ${BL1_DIR}/bl1-${TARGET_SOC} nxp4330 clova 3
fi
...
if [ "${BUILD_ALL}" == "true" ] || [ "${BUILD_KERNEL}" == "true" ]; then
	build_kernel ${KERNEL_DIR} ${TARGET_SOC} ${BOARD_NAME} s5p4418_clova_nougat_defconfig ${CROSS_COMPILE}
	test -d ${OUT_DIR} && \
...
if [ -f ${UBOOT_DIR}/u-boot.bin ]; then
	...
	UBOOT_BOOTARGS="console=ttyAMA3,115200n8 loglevel=7 printk.time=1 androidboot.hardware=clova androidboot.console=ttyAMA3 androidboot.serialno=s5p4418_clova quiet"
...
make_ext4_recovery_image \
	...
	${KERNEL_DIR}/arch/arm/boot/dts/s5p4418-clova-rev00.dtb \
